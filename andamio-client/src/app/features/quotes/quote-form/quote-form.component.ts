  ...
  onSubmit() {
    ...
    const allItems = [
      ...this.quoteForm.value.laborItems.map((i: any) => ({ 
        ...i, 
        type: 'mano_de_obra', // Cambiamos 'labor' por el valor del ENUM en tu tabla
        total_price: (+i.unit_price * +i.quantity) // Calculamos el total_price para la tabla
      })),
    ...
    this.quoteService.createQuote(finalData).subscribe({
      ...
      error: (err) => {
        ...
        alert('❌ Error al guardar la cotización. Revisa la conexión con MySQL.');
      }
    });
  }
